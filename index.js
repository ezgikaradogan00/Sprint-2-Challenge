...
function foo(degisken="Hello World!") {
	return degisken;
}
console.log(foo());
...
/* 	GÖREV 1:
	...
	*/
	...
	function cumlelereDonustur(cumleDizisi, ayrac = ",") {
		let dizi = cumleDizisi.map((item) => item.join(ayrac));
		return dizi;
	  }
...
/* GÖREV 2:
	...
	*/
	...
	function paragrafOlustur(cumleDizisi,callbackCumleKur,callbackCumlelereDonustur) {
		let dizi = callbackCumlelereDonustur(cumleDizisi, " ");
		let paragraf = callbackCumleKur(dizi[1],dizi[3],dizi[5],dizi[7],dizi[9]);
		return paragraf;
	  }
